-- Add optional phone storage for leads across UI and OpenClaw ingestion.
alter table public.leads
add column if not exists phone text;

-- Keep OpenClaw create idempotent while also allowing phone enrichment.
...
begin
  ...
  set
    ...
    phone = case
      when coalesce(btrim(existing.phone), '') = '' then nullif(btrim(coalesce(p_phone, '')), '')
      else existing.phone
    end,
    niche = case
